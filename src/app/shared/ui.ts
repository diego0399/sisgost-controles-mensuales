...
export function estadoKind(estado: string): 'ok' | 'warn' | 'danger' | 'info' | 'gold' | 'neutral' {
  ...
  if (/(vencid|sin asignar|sin soporte|presenta falla|no cumple|incumpl|descargado|no disponible)/.test(e)) return 'danger';
  if (/(observad|pendiente de revisión|con hallazgos|en garantía|parcial|en remediación)/.test(e)) return 'warn';
  if (/justificad/.test(e)) return 'gold';
  if (/(inactiv|no aplica|finalizada|sin cambios|sin garantía)/.test(e)) return 'neutral';
  if (/(entregad|enviad|cerrad|cumple|aceptad|aplicado|resuelto|activo|activa|completad|realizada|verificado|registrada|funciona|al día|emitida|generado|descargado el|vigente|bueno|actualizada|up)/.test(e)) return 'ok';
